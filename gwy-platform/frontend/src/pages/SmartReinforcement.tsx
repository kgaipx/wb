    ...
    <section>
      ...
      {weak.length > 0 && (
        ...
        <div className="card re-cta">
          ...
          <button className={"btn btn--primary re-cta__btn" + (busy === "all" ? " btn--loading" : "")} disabled={busy !== null} onClick={reinforceAll}>
            {busy === "all" && <Spinner size={15} />}
        ...
        <div className="re-kp-list">
          {weak.map((a, i) => (
            ...
            <div className="card re-kp">
              ...
              <button
                className={"btn btn--ghost btn--sm re-kp__btn" + (busy === a.knowledge_point ? " btn--loading" : "")}
                disabled={busy !== null}
                onClick={() => reinforceKp(a.knowledge_point)}
              >
                {busy === a.knowledge_point && <Spinner size={14} />}
        ...
        <div className="re-wrong-list">
          {wrongs.slice(0, 30).map((w, i) => (
            ...
            <div className="card re-wrong">
              ...
              <button
                className={"btn btn--ghost btn--sm re-kp__btn" + (busy === "w" + w.question.id ? " btn--loading" : "")}
                disabled={busy !== null}
                onClick={() => reinforceFromWrong(w)}
              >
                {busy === "w" + w.question.id && <Spinner size={14} />}
